...
use crate::{input::mouse_position_vec, physical_card::PhysicalCard};
    ...
    pub fn step(&mut self, dt: f32) {
        ...
        if is_mouse_button_pressed(MouseButton::Left) {
            ...
            self.holding = pop_where(&mut self.deck, |physical_card| {
                point_inside(mouse_position_vec(), &physical_card.transform)
            })
            .or(self
                .card_pool
                .iter()
                .find(|physical_card| point_inside(mouse_position_vec(), &physical_card.transform))
                .cloned());
        }
        ...
        for physical_card in self.deck.iter_mut().chain(self.card_pool.iter_mut()) {
            let scale = if point_inside(mouse_position_vec(), &physical_card.transform) {
                1.2
            } else {
                1.0
            };
        ...
        if is_mouse_button_released(MouseButton::Left) {
            if let Some(holding) = &self.holding {
                if mouse_position_vec().x > screen_width() / 2.0 {
