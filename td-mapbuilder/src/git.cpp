...
bool gitHandler::initializeRepository(git_repository* repo, int r_value) {
if (r_value != 0) {
...
gitHandler::gitHandler(std::string project_path) : repository_path(std::move(project_path))
{
...
bool gitHandler::gitInitWrapper(std::function<bool(git_repository* repo, int r_value)> func)
{
    git_libgit2_init();
...
bool gitHandler::initialCommit(git_repository* repo){
...
bool gitHandler::stageAndCommit(const std::vector<std::string> stage_files)
{
...
bool gitHandler::LoadCommit(git_oid commit)
{
